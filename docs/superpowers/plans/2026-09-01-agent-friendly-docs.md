...
**The spec branch is GONE — base on `main`.** `docs/launch-1.8-agent-docs-spec` was merged (merge commit `fe3e6cc`) and deleted after the plan's first draft was written; `git branch -a --list '*launch-1.8*'` is empty both locally and on `origin`. Verified 2026-09-01: `git merge-base --is-ancestor 8c08894 main` succeeds, so the spec (and this plan's source of truth) is already reachable from `main`. Branching from the deleted name fails with `fatal: invalid reference`. Expected output above: `main`, with `fe3e6cc` in the log and an EMPTY branch list. If the branch list is NOT empty, someone recreated it — stop and reconcile before continuing.
...
Questions 1 and 5 were put to the user on 2026-09-01 and are **resolved** (marked inline below); the runbook's silence on trial terms is now a *decision*, not an omission, and the selftest that pins it is enforcing an agreed constraint. Questions 2-4 remain open and are for the plan review still in progress — none blocks starting Task 1, and each is a one-line change if the answer comes back differently.
